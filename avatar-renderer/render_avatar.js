...
(async () => {
    ...
    <body>
        ...
        <script type="module">
            ...
            // Persona static matrices
            const PERSONA_VRM0 = {
                breathFreq: 1.75,
                breathAmp: 0.016,
                weightShiftPeriod: 8.5,
                hipSwayAmp: 0.007,
                hipTiltBase: -0.045,
                hipTiltAmp: 0.012,
                spineTilt: 0.035,
                ...
                headRollAmp: 0.014,
                headPitchBias: -0.025,
                saccadeIntervalMin: 1.8,
                saccadeIntervalVar: 2.2,
                blinkPeriod: 3.5,
                baselineJoy: 0.18,
                baselineFun: 0.22
            };
            ...
            const PERSONA_VRM1 = {
                ...
                headRollAmp: 0.007,
                headPitchBias: -0.040,
                saccadeIntervalMin: 3.4,
                saccadeIntervalVar: 3.0,
                blinkPeriod: 4.2,
                baselineJoy: 0.06,
                baselineFun: 0.08
            };
            ...
            function createStudioEnvironment() {
                ...
                const floorMesh = new THREE.Mesh(floorGeo, new THREE.MeshBasicMaterial({ map: floorTex }));
                ...
                sGrad.addColorStop(0, 'rgba(15, 23, 42, 0.55)');
                sGrad.addColorStop(0.40, 'rgba(30, 41, 59, 0.25)');
                sGrad.addColorStop(0.75, 'rgba(51, 65, 85, 0.06)');
                ...
                const floorShadow = new THREE.Mesh(
                    ...
                    new THREE.MeshBasicMaterial({ map: sTex, transparent: true, opacity: 0.85, depthWrite: false })
                );
                ...
                const bgMesh = new THREE.Mesh(new THREE.PlaneGeometry(30, 16), new THREE.MeshBasicMaterial({ map: bgTex }));
            ...
            async function init() {
                ...
                if (debugInfo) {
                    console.log("[Chromium GPU Context] " + gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL));
                }
                ...
                gltfLoader.load('/avatar.vrm', (gltf) => {
                    ...
                    if (!loadedVrm) {
                        console.error("VRM initialization error: No VRM instance found in gltf.userData");
                        return;
                    }
                    ...
                    const EXPR_ALIAS_MAP = {
                        'a': 'aa', 'i': 'ih', 'u': 'ou', 'e': 'ee', 'o': 'oh',
                        'joy': 'happy', 'fun': 'relaxed', 'sorrow': 'sad',
                        'aa': 'a', 'ih': 'i', 'ou': 'u', 'ee': 'e', 'oh': 'o',
                        'happy': 'joy', 'relaxed': 'fun', 'sad': 'sorrow'
                    };
                    ...
                    if (!vrm.blendShapeProxy && vrm.expressionManager) {
                        vrm.blendShapeProxy = {
                            setValue: function(name, value) {
                                ...
                                const alt = EXPR_ALIAS_MAP[name];
                                if (alt) {
                                    try { vrm.expressionManager.setValue(alt, val); } catch (_) {}
                                }
                            },
                    ...
                    console.log("[Chromium] VRM model initialized and ready.");
                    window.__ready = true;
            ...
            window.updateFrame = function(t, phoneme, expr, camMode, camAge, activeOverlay, overlayPos, activeGesture, gestAge, ltTitle, ltAge) {
                ...
                if (cam) {
                    ...
                    let basePos = POS_FULL;
                    let baseLook = LOOK_FULL;
                    let isSlow = false;
                    ...
                    if (isSlow) {
                        ...
                        cam.position.lerp(targetPos, 0.35);
                        window.__camLook.lerp(targetLook, 0.35);
                    }
                ...
                if (hips) {
                    ...
                    hips.rotation.z = persona.hipTiltBase + weightShift * persona.hipTiltAmp;
                ...
                if (chest) {
                    chest.rotation.x = 0.015 + breathCycle * persona.breathAmp;
                ...
                if (isVRM0) {
                    ...
                    if (rArm && rElbow) {
                        ...
                        const pointArmX = -0.30;
                        const pointArmY = -0.25;
                        const pointArmZ = 0.45;
                        const pointElbX = -0.10;
                        const pointElbY = 0.0;
                        const pointElbZ = 0.0;
                ...
                if (activeEmotionKey === 'neutral' && vrm.blendShapeProxy) {
                    vrm.blendShapeProxy.setValue('joy', persona.baselineJoy);
                    vrm.blendShapeProxy.setValue('fun', persona.baselineFun);
                }

                // Native vrm.update calculates skeletal joints, spring bones, and expressions in a single pass
                vrm.update(1 / 30);
    ...
    page.on('console', msg => console.log(`LOG:[Chromium ${msg.type()}] ${msg.text()}`));
    page.on('pageerror', err => console.error(`LOG:[Chromium Exception] ${err.message}`));

    await page.setViewport({ width: 1280, height: 720 });

    await page.goto(`http://127.0.0.1:${serverPort}/`, { waitUntil: 'domcontentloaded', timeout: 120000 });
